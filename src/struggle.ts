  ...
  export default class Struggle {
    ...
    constructor(options) {
      ...
      if (!options.provider) throw new Error('Missing provider value (mongo, postgres, sqlite)');
    ...
    destroy() {
      switch (this.provider) {
        ...
        case 'postgres': {
          if (this.db === null) throw new Error('Unable to close the connect. No database connection is active.');
          this.db.end();
    ...
    select(value) {
      if (!value) throw new Error('Missing vlaue');
  
      // If method already exists, return error
      if (this.method != null) throw new Error(`Can't SELECT and ${this.method}.`); // No clue how to phrase this
      if (!this.chainable) throw new Error(`Lol can't chain`);
    ...
    create(value) {
      // If method already exists, return error
      if (this.method != null) throw new Error(`Can't CREATE and ${this.method}.`); // No clue how to phrase this
      if (!this.chainable) throw new Error(`Lol can't chain`);
    ...
    drop(value) {
      // Drop tables
      if (this.method != null) throw new Error(`Can't DROP and ${this.method}.`); // No clue how to phrase this
      if (!this.chainable) throw new Error(`Lol can't chain`);
    ...
    delete(value) {
      ...
      if (this.method != null) throw new Error(`Can't DELETE and ${this.method}.`); // No clue how to phrase this
      if (!this.chainable) throw new Error(`Lol can't chain`);
    ...
    // COUNT()
    count() {
      if (this._count) throw new Error('Count already activated nub.');
      if (!this.chainable) throw new Error(`Lol can't chain`);
